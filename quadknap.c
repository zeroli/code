...
typedef struct ilist {
  ...
  ntype  no;       the items position in a pw ordering 
} itemlist;
...
{
  printf(%sn, str);
  printf(PROGRAM IS TERMINATED !!!nn);
  exit(-1);
}
...
void improve(int xprime)
{
  ...
  res = c;
   printf(improve n %d res %d c %dn, n, res, c); 
  ...
  for (;;) {
    ...
    for (i = 0; i  n; i++) {
     ...
     printf(best gain %d i %d j %d res %dn, bgain, gaini, gainj, res); 
    if (bgain == 0) break;
    xprime[gaini] = 1;
  ...
  if (res  0) error(Excess capacity);
   printf(possible solution %dn, gain); 
  if (gain  z) {
     printf(improved solution %d to %dn, z+fixp, gain+fixp); 
    z = gain;
...
void greedy(void)
{
  ...
  for (;;) {
    ...
    if (mini == -1) error(No item foundn);
    i = mini; x[i] = 0;
    psum -= minp; wsum -= w[i];
    if (wsum = c) break;
  }
  z = heur = psum;
  memcpy(xstar, x, MSIZEsizeof(boolean));
   printf(greedy psum %d wsum %d c %dn, psum, wsum, c); 
  improve(x);
}
...
void iterate(int loopno)
{
  ...
  for (h = 0; h  maxlagr; h++) {
    ...
    zlp = lpsolve(a, c, n);
     if (h % 50 == 0) printf(%d bound %dn, h, zlp+fixp);  
  ...
  lagbound = zlpmin+fixp; z0 = z+fixp;
   printf(best bound %d greedy z %d impr z %dn, lagbound,heur,z+fixp); 
}
    ...
    if (xstar[i] == 1) {
      ...
      if (u  z+1) {
        ord[i] = i+n; xstar[i] = 1; r1++;
         printf(item %d fixed at 1n, i); 
      }
    } else {
      u = findbound(0, c, i, 1);
      if (u  z+1) {
        ord[i] = i+n; xstar[i] = 0; r0++;
         printf(item %d fixed at 0n, i); 
      }
    }
  }
  qsort(ord, n, sizeof(ntype), (funcptr) icomp);
  for (i = 0; i  n; i++) if (ord[i] = n) ord[i] -= n;
   printf(fixed %d at 1, %d at 0 out of %dn, r1, r0, n); 
  return r0 + r1;
}
...
void initsets(void)
{
  ...
  for (i = 0; i  n; i++) {
    ...
    row-tail.p = 0; row-tail.w = c;
     printf(row kp (%d,%d) c %d b (%d,%d)n, psum,wsum,c1,h-p,h-w); 
  }
}
...
void removeitem(int t)
{
  ...
  for (i = t+1, row = rowk+t+1; i  n; i++, row++) {
    h = &(row-it[t]);
     printf(%d removing (%d,%d) no %dn, i, h-p, h-w, h-no); 
    h-next-prev = h-prev;
    h-prev-next = h-next;
    h-psumb = row-psumb;
    h-wsumb = row-wsumb;
...
void insertitem(int t)
{
  ...
  for (i = t+1, row = rowk+t+1; i  n; i++, row++) {
    h = &(row-it[t]);
     printf(inserting (%d,%d)n, h-p, h-w); 
    h-next-prev = h;
    h-prev-next = h;
    row-b     = h-b; 
    row-psumb = h-psumb; 
...
stype breakbound(stype ps, stype ws, int t)
{
  ...
  stype u, psum;
   ...
   printf(breakbound ps %d, ws %d, c1 %d t %dn, ps, ws, c-ws, t); 
  for (i = t, k = d, row = rowk+t; i  n; i++, row++) {
    ...
    row-b = h; row-psumb = psumb; row-wsumb = wsumb; 
     printf( %d found c1 %d b (%d,%d)n, i, c1, h-p, h-w); 
  ...
  u = psum + ps;    fprintf(trace,derived u %d = %dn, u, u+fixp); 
  return u;
}
...
void quadbranch(stype ps, stype ws, int t)
{
  ...
  if (ps  z) {
    z = ps; 
     printf(improved solution to %dn, z); 
...
int quadknap(int no, int cap, int ptab, int wtab, int xtab)
{
  ...
  for (i = 1;; i++) {
    ...
    iterate(i);
     ...
     perform reduction 
    fix = reduce(ord);
     printf(%d NOW REDUCED %d TOTAL REDUCED %dn, i, fix, nf+fix); 
    if (fix == 0) break;
    reorder(ord, o1);
    decrease(fix, &ps, &ws);
    nf += fix; n -= fix; z -= ps; c -= ws; fixp += ps; fixw += ws;
  }
   printf(inibound %d, lagbound %d, greedy z %dn, inibound,lagbound,z); 
  ...
  quadbranch(0, 0, 0); 
   ...
   restore table 
   printf(increase n by %d, z by %d, c by %dn, nf, fixp, fixw); 
  increase(nf);
  n += nf; z += fixp; c += fixw;
  invertorder(ord, o1);
  reorder(ord, o1);
